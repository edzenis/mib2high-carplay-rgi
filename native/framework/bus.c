 ...
 * listening on 127.0.0.1:19810.  This matches the process lifetime
 * hierarchy: Java is alive from boot and mm-ipod is spawned per phone connect.
 *
 * Threads:
 ...
 *
 * This intentionally uses the proven mutex-backed queue required by the
 * target QNX runtime.
 * ============================================================ */
 ...
 * by design for target QNX reliability.
 * ============================================================ */
...
static void k2161_bus_block_sigpipe(void);

static void* writer_main(void* arg) {
    (void)arg;
    k2161_bus_block_sigpipe();
...
static int try_connect_once(void) {
    ...
    /* 500ms send timeout so a stalled Java peer (TCP buffer full or blocked
     * server) cannot pin the writer indefinitely inside
...
/*
 * K2161/mm-ipod compatibility:
 * socket writes may raise SIGPIPE on QNX 6.5, but changing SIGPIPE globally
 * would alter stock mm-ipod behaviour.  Block it only in our bus threads.
 */
static void k2161_bus_block_sigpipe(void) {
    sigset_t set;
    sigemptyset(&set);
...
static void* connector_main(void* arg) {
    ...
    while (!g_shutdown) {
        ...
        /* Send HELLO directly before exposing the fd to the async writer so
         * the first bytes on a fresh socket do not depend on the ring queue
         * or writer-thread scheduling. */
 ...
 *
 * ASYNC-SIGNAL-SAFE ONLY.  Use one write(2) of a fixed string, restore the
 * default handler, and re-raise so normal core-dump behavior is preserved. */
...
hook_result_t bus_init(void) {
    ...
    /* K2161/mm-ipod: preserve host process SIGPIPE disposition. */
    ...
    /* K2161/mm-ipod: preserve stock host fault-signal disposition. */
    /* K2161/mm-ipod: preserve stock host fault-signal disposition. */
    /* K2161/mm-ipod: preserve stock host fault-signal disposition. */
    /* K2161/mm-ipod: preserve stock host fault-signal disposition. */
    /* K2161/mm-ipod: preserve stock host fault-signal disposition. */
 ...
 * replace any byte < 0x20 with a space. UTF-8 multibyte sequences and
 * ordinary punctuation pass through. */
