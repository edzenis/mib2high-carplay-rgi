 ...
 *
 * Threading model:
 *   - bus_init() starts a connector thread and a writer thread.
 *   - the native hook is the TCP client; Java listens on 127.0.0.1:19810.
 *   - bus_send() is thread-safe and queues data for the writer thread, so the
 *     iAP2 hook path does not perform network writes directly.
 *   - bus_on() registers inbound command handlers; callbacks run on the
 *     connection reader path and must return quickly.
 *
