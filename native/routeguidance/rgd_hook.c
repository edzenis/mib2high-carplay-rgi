 ...
 *
 * Native iAP2 can absorb that reset cheaply.  The Java side owns BAP guidance
 * lifecycle, so forwarding the intermediate zero tears down HUD guidance and
 * causes visible flicker.  Treat state=0 as pending
 * until either:
...
static struct {
    ...
    /* Maneuver storage mapping:
     * iOS uses monotonically increasing maneuver indexes (can exceed small caches).
     * The reference stack stores a bounded vector and drops the oldest entry when capacity is exceeded.
     *
...
static int rgd_min_current_index(void) {
	...
	static bool rgd_can_process_maneuver_index(uint16_t idx) {
	    /* Mirror isRouteGuidanceManeuverIndexGreater semantics from the reference implementation (simplified):
...
static void write_bus_maneuver_partial(const rgd_maneuver_t* man) {
    ...
    if (slot < 0) {
        rgi_diag_log("CACHE", "reject raw_index=%u reason=no_slot", iap_idx);
        return;
    }
    rgi_diag_log("CACHE", "cache_store raw_index=%u slot=%d present=0x%llx type=%u list_present=%d list_count=%u",
                iap_idx, slot, (unsigned long long)man->present, (unsigned)man->maneuver_type,
                g_rgd.current_list_present ? 1 : 0, (unsigned)g_rgd.current_list_count);
...
static void write_bus_snapshot_from_cache(int extra_slot, const rgd_maneuver_t* extra_man,
    ...
    /* Heap allocation is intentional: gcc 4.4 on QNX 6.5 emits __thread as
     * emulated TLS (__emutls_v.* + __emutls_get_address), which is unsafe in
     * this LD_PRELOAD context on the tested target. */
    ...
    bus_text_uint(b, "diag_session", rgi_diag_session_get());
    bus_text_uint(b, "diag_rx_seq", rgi_diag_current_rx_seq());
    ...
    /* Publish valid cached 0x5202 slot data even before it becomes current.
     * This is storage transport only: maneuver_list remains the sole authority
     * for presentation order/current maneuver.  Never infer current from RX order. */
    if (have_extra_slot) {
        bool already_listed = false;
        for (int i = 0; i < listed_count; i++) {
            if (listed_slots[i] == extra_slot) { already_listed = true; break; }
        }
        if (!already_listed) {
            write_slot_data_keys(b, (unsigned)extra_slot, extra_man);
            bus_text_int(b, "cached_extra_slot", extra_slot);
            rgi_diag_log("CACHE", "publish_cached_noncurrent slot=%d raw_index=%u present=0x%llx list_present=%d list_count=%u",
                        extra_slot, (unsigned)g_rgd.slot_to_iap_idx[extra_slot],
                        (unsigned long long)extra_man->present,
                        g_rgd.current_list_present ? 1 : 0,
                        (unsigned)g_rgd.current_list_count);
        }
    }
...
static bool rgd_message_handler(hook_context_t* ctx, const iap2_frame_t* frame) {
    ...
    if (frame->msgid == IAP2_MSG_ROUTE_GUIDANCE_UPDATE) {
        ...
        {
            char maneuver_list_text[128];
            int list_off = 0;
            maneuver_list_text[0] = '\0';
            for (uint16_t list_i = 0; list_i < upd.maneuver_list_count; list_i++) {
                list_off += snprintf(maneuver_list_text + list_off, sizeof(maneuver_list_text) - (size_t)list_off,
                                   "%s%u", list_i ? "," : "", (unsigned)upd.maneuver_list[list_i]);
                if (list_off >= (int)sizeof(maneuver_list_text) - 4) break;
            }
            rgi_diag_log("5201",
                        "RGI 5201 present=0x%llx state_p=%d state=%u count_p=%d count=%u list_p=%d list_count=%u list=%s dist_p=%d dist=%u src_p=%d src=%u visible_p=%d visible=%u",
                        (unsigned long long)upd.present,
                        (upd.present & RGD_UPD_ROUTE_STATE) ? 1 : 0, (unsigned)upd.route_state,
                        (upd.present & RGD_UPD_MANEUVER_COUNT) ? 1 : 0, (unsigned)upd.maneuver_count,
                        (upd.present & RGD_UPD_MANEUVER_LIST) ? 1 : 0, (unsigned)upd.maneuver_list_count, maneuver_list_text,
                        (upd.present & RGD_UPD_DIST_TO_MANEUVER) ? 1 : 0, (unsigned)upd.dist_to_maneuver,
                        (upd.present & RGD_UPD_SOURCE_SUPPORTS_RG) ? 1 : 0, (unsigned)upd.source_supports_route_guidance,
                        (upd.present & RGD_UPD_VISIBLE_IN_APP) ? 1 : 0, (unsigned)upd.visible_in_app);
        }
        ...
        if (upd.present & RGD_UPD_ROUTE_STATE) {
            ...
            if (upd.route_state == 0) {
                rgi_diag_log("END", "route_state_zero_rx prev=%u emitted=%u hard_clear=%d pending_started=%llu",
                            (unsigned)prev_state, (unsigned)g_rgd.emitted_route_state, hard_clear ? 1 : 0,
                            (unsigned long long)g_rgd.state_zero_started_ms);
                if (hard_clear) {
        ...
        rgi_diag_log("5202",
                    "RGI 5202 present=0x%llx index=%u type=%u exit_angle=%d junction=%u side=%u distance_between=%u desc=%s after=%s",
                    (unsigned long long)man.present, (unsigned)man.index,
                    (unsigned)man.maneuver_type, (int)man.exit_angle,
                    (unsigned)man.junction_type, (unsigned)man.driving_side,
                    (unsigned)man.distance_between, man.description, man.after_road_name);
...
static size_t rgd_identify_patcher(hook_context_t* ctx, uint8_t* buf, size_t len, size_t max_len) {
    ...
    if (new_len > max_len) {
        /* Grow the NmeArray storage instead of skipping. The tested target's
         * array storage is malloc-compatible, and the component must fit for
         * the phone to learn that route guidance is supported. */
        size_t new_cap = 0;
...
static void rgd_state_handler(hook_context_t* ctx, int event, void* event_data) {
    ...
    switch (event) {
        ...
        case HOOK_EVENT_IDENTIFY_END:
            /* IdentifyEnd means the phone accepted the advertised component.
             * Preserve component_valid; the heartbeat requests/retries as soon
