...
static void copy_printable_string_preview(const uint8_t* data, int len, char* out, size_t out_sz) {
     ...
     *
     * Exit info in the reference implementation is fed into util::UnicodeString8, so it can contain UTF-8.
...
void rgd_parse_maneuver(const uint8_t* buf, size_t len, rgd_maneuver_t* out) {
    ...
    /*
     * Reference route-guidance processor (CRouteGuidanceUpdateProcessorImpl::getRouteGuidanceManeuverInformation)
...
size_t rgd_build_component_tlv(uint8_t* out, size_t max_len, uint16_t component_id) {
    ...
    /*
     * Match the reference RouteGuidanceDisplayComponent layout (9 TLVs).
     *
     ...
     *
     * IDs 7 and 8 must remain the lane-guidance description/storage
     * capacities.  Moving them or adding unsupported capability fields can
     * suppress 0x5204 LaneGuidance delivery.
     */
