...
#define ALT111_MODULE "AltScreen111"
#define ALT111_MAX_STATES 4
#define ALT111_TYPE 111
#define ALT111_TIMEOUT_SECS 10
#define K_CFSTRING_UTF8 0x08000100U

/* Keep these byte-identical to altscreen_airplay.c. */
#define ALT111_UUID "E0CB6FB0-0000-0000-0000-0000C0FFEE58"
#define ALT111_INITIAL_URL "maps:/car/instrumentcluster/map"
...
static alt111_api g_api;
static alt111_state g_states[ALT111_MAX_STATES];
...
static cf_ref build_stock_only_request(cf_ref request, cf_ref *out_alt_desc)
{
    ...
    if (alt_count != 1) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 split refused count=%d expected=1 main110_untouched=1",
                  alt_count);
        goto fail;
    }
...
static cf_ref merge_private_response(cf_ref stock_response, uint16_t port)
{
    cf_ref response = NULL;
    cf_ref streams = NULL;
    cf_ref out_streams = NULL;
    cf_ref private_stream = NULL;
    cf_ref enabled = NULL;
    ...
    private_stream = new_dict();
    if (out_streams == NULL || private_stream == NULL)
        goto fail;

    if (!dict_set_i64(private_stream, "type", ALT111_TYPE) ||
        !dict_set_i64(private_stream, "dataPort", (int64_t)port))
        goto fail;

    g_api.array_append(out_streams, private_stream);
    ...
    g_api.release(private_stream);
    g_api.release(out_streams);
    return response;

fail:
    if (out_enabled != NULL) g_api.release(out_enabled);
    if (private_stream != NULL) g_api.release(private_stream);
...
static alt111_state *find_state_receiver_locked(void *receiver)
{
    int i;

    for (i = 0; i < ALT111_MAX_STATES; ++i) {
...
static alt111_state *find_state_worker_locked(pthread_t thread)
{
    int i;
...
static alt111_state *alloc_state_locked(void *receiver)
{
    int i;
...
static cf_ref make_event_request(const char *type, int include_url)
{
    ...
    if (!dict_set_string(request, "type", type) ||
        !dict_set_string(params, "uuid", ALT111_UUID))
        goto fail;
    if (include_url && !dict_set_string(params, "url", ALT111_INITIAL_URL))
        goto fail;
...
static os_status send_event(void *receiver, const char *type, int include_url)
{
    ...
    LOG_INFO(ALT111_MODULE,
             "ALT111 event type=%s rc=%d uuid=%s url=%s",
             type, rc, ALT111_UUID, include_url ? ALT111_INITIAL_URL : "-");
    return rc;
}

static void request_cluster(alt111_state *state)
{
    os_status rc;
...
{
    alt111_state *state = (alt111_state *)arg;
    ...
    LOG_INFO(ALT111_MODULE,
             "STREAM_111 worker waiting receiver=%p port=%u",
    ...
    if (fd < 0) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 accept failed receiver=%p errno=%d",
                  state->receiver, errno);
        return NULL;
    }

    delegate_context = k2161_altscreen_main_delegate_context();
    rc = k2161_altscreen_private_start(state->screen, delegate_context);
    if (rc != 0) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 private start failed receiver=%p rc=%d",
                  state->receiver, rc);
        close(fd);
        pthread_mutex_lock(&g_lock);
        state->accepted_fd = -1;
    ...
    if (rc != 0 || net == NULL) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 NetSocket create failed receiver=%p rc=%d",
                  state->receiver, rc);
        close(fd);
        pthread_mutex_lock(&g_lock);
        state->accepted_fd = -1;
    ...
    LOG_INFO(ALT111_MODULE,
             "STREAM_111 processing begin receiver=%p screen=%p delegate=%p",
             state->receiver, state->screen, delegate_context);

    rc = g_api.screen_process(state->screen, net, ALT111_TIMEOUT_SECS);
    ...
    LOG_INFO(ALT111_MODULE,
             "STREAM_111 processing end receiver=%p rc=%d",
             state->receiver, rc);
    return NULL;
}

static int prepare_private111(void *receiver, cf_ref alt_desc, uint16_t *out_port)
{
    alt111_state *state = NULL;
    void *screen = NULL;
    uint8_t key[16];
    uint8_t iv[16];
    uint64_t connection_id;
    ...
    if (!ok || connection_id == 0) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 prepare refused reason=missing_connection_id");
        return 0;
    }

    if (!k2161_altscreen_private_aes_ready()) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 prepare refused conn=%llu reason=stock_AES_not_ready main110_preserved=1",
    ...
    if (state == NULL) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 prepare refused receiver=%p reason=state_collision_or_full",
                  receiver);
        return 0;
    }
    ...
    if (!k2161_altscreen_derive_private_aes(connection_id, key, iv))
        goto fail;
    ...
    if (!k2161_altscreen_register_private_screen(screen))
        goto fail;
    ...
    rc = pthread_create(&state->worker, NULL, private_worker, state);
    if (rc != 0) {
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 worker create failed receiver=%p rc=%d",
                  receiver, rc);
        goto fail_listener;
    }
    ...
    *out_port = port;
    LOG_WARN(ALT111_MODULE,
             "STREAM_111 PRIVATE_READY receiver=%p screen=%p conn=%llu dataPort=%u main110_untouched=1",
...
fail_listener:
    ...
    pthread_mutex_unlock(&g_lock);
fail_registered:
    k2161_altscreen_unregister_private_screen(screen);
fail:
    if (screen != NULL)
...
static void cleanup_private111(void *receiver)
{
    alt111_state *state;
    pthread_t worker;
    int worker_started;
    ...
    LOG_INFO(ALT111_MODULE,
             "STREAM_111 cleanup receiver=%p route_stop=%d",
             receiver, show_sent);
}
                                      ...
                                      cf_ref *out_response)
{
    ...
    if (!k2161_altscreen_enabled() || !g_api.ready || out_response == NULL)
    ...
    LOG_INFO(ALT111_MODULE,
             "STREAM_111 requested receiver=%p stock_only_request=%p descriptor=%p main110_protected=1",
    ...
    if (rc != 0) {
        ...
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 stock setup failed receiver=%p rc=%d",
                  receiver, rc);
        return rc;
    }

    if (!prepare_private111(receiver, alt_desc, &private_port)) {
        *out_response = stock_response;
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 private prepare failed receiver=%p stock110_response_preserved=1",
                  receiver);
        return 0;
    }

    final_response = merge_private_response(stock_response, private_port);
    if (final_response == NULL) {
        cleanup_private111(receiver);
        *out_response = stock_response;
        LOG_ERROR(ALT111_MODULE,
                  "STREAM_111 response merge failed receiver=%p stock110_response_preserved=1",
                  receiver);
        return 0;
    }
    ...
    *out_response = final_response;
    ...
    LOG_WARN(ALT111_MODULE,
             "STREAM_111 SETUP_READY receiver=%p dataPort=%u enabledFeatures=altScreen,viewAreas main110_untouched=1",
             receiver, (unsigned int)private_port);
    return 0;
}
...
{
    resolve_api();
    if (k2161_altscreen_enabled() && g_api.ready)
        cleanup_private111(receiver);
                                  ...
                                  void *completion_context)
{
    ...
    if (rc != 0 || !k2161_altscreen_enabled())
        return rc;
    ...
    if (should_show) {
        LOG_WARN(ALT111_MODULE,
                 "NATIVE_111_FIRST_REAL_FRAME receiver=%p stream=%p len=%u target=58 context=76",
