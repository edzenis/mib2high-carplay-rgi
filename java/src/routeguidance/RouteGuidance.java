...
public class RouteGuidance implements CarplayBus.Listener {
    ...
    /*
     * RouteGuidanceState values verified from reference route-guidance type information:
    ...
    /*
     * Reference route-guidance processor behavior (CRouteGuidanceUpdateProcessorImpl::isRouteGuidanceManeuverIndexGreater):
     * if routeGuidanceState == 5 then maneuver updates are always accepted.
     *
     * Naming in the available navigation references varies (Locating/Rerouting); value 5 matches the observed state check.
     */
    ...
    public static class State {
         ...
         * The reference route-guidance processor uses sentinel 1000 when the field is missing.
         */
    ...
    /* Pure extraction of the existing wantActive decision for host verification. */
    static boolean wantsRouteOwnership(State candidate) {
        boolean explicitClear = ((candidate.dirtyMask & State.DIRTY_MANEUVER_COUNT) != 0)
            && (candidate.maneuverCount == 0);

        boolean routeLooksActive = (candidate.routeState >= ROUTE_STATE_ROUTE_SET)
            || (candidate.maneuverCount > 0)
            || (candidate.maneuverOrder != null && candidate.maneuverOrder.length > 0);

        boolean wantActive;
        if (candidate.visibleInApp >= 0) {
            wantActive = (candidate.visibleInApp != 0) || routeLooksActive;
        } else {
            wantActive = routeLooksActive;
            if (explicitClear && candidate.routeState < ROUTE_STATE_ROUTE_SET) {
                wantActive = false;
            }
        }

        if (candidate.sourceSupportsRg == 0) wantActive = false;
        if (candidate.routeState == ROUTE_STATE_NO_ROUTE_SET) wantActive = false;
        return wantActive;
    }
    ...
    public void onFrame(int type, int flags, byte[] payload, int len) {
        ...
        parse(d);
        int diagSession = d.num("diag_session", -1);
        int diagRx = d.num("diag_rx_seq", -1);
        int orderLen = state.maneuverOrder == null ? -1 : state.maneuverOrder.length;
        int order0 = orderLen > 0 ? state.maneuverOrder[0] : -1;
        Log.d(TAG, "bus session=" + diagSession + " rx=" + diagRx
            + " dirty=0x" + Integer.toHexString(state.dirtyMask)
            + " state=" + state.routeState + " count=" + state.maneuverCount
            + " orderLen=" + orderLen + " order0=" + order0
            + " dist=" + state.distManeuverM);
         ...
         *
         * Change RG active state only after receiving an activation-relevant delta.
         */
        ...
        if (hasActivationDelta) {
            boolean wantActive = wantsRouteOwnership(state);
            ...
            if (wantActive && !rgActive) {
                ...
                if (bap != null && bap.onStart()) {
                    ...
                    Log.w(TAG, "RG activation deferred: ownership gate not ready");
                }
        ...
        if (bap != null) {
            if (state.maneuverOrder == null || state.maneuverOrder.length == 0) {
                Log.d(TAG, "present-skip reason=no_authoritative_order"
                    + " count=" + state.maneuverCount + " route_state=" + state.routeState);
            }
            bap.update(state);
            state.clearDirty();
        }
    ...
    private void parse(CarplayBus.Data d) {
        ...
        if (d.has("source_supports_rg")) {
            ...
            if (v != state.sourceSupportsRg) {
                ...
                /*
                 * The reference route-guidance processor uses the "source supports route guidance" signal to suppress
                 * route guidance updates. For the HUD bridge, treat support==0 as a hard clear
        ...
        if (d.has("route_state")) {
             ...
             *
             * Perform a full per-slot clear only when there is positive evidence of a
        ...
        if (d.has("maneuver_state")) {
            ...
            /* Reference EManeuverState: 0..3 are valid (CONTINUE/INITIAL/PREPARE/EXECUTE). */
        ...
        if (d.has("maneuver_count")) {
            ...
            if (v != state.maneuverCount) {
                ...
                if (v == 0) {
                     ...
                     * changes, so clearing Java state would prevent the bus from re-sending unchanged values,
                     * losing the data until a new 0x5202 arrives.
                     *
                     * maneuver_count=0 can be transient - iOS briefly clears the count before
                     * re-populating (observed at route start). Preserving slot data avoids
        ...
        for (int i = 0; i < MAX_MANEUVERS; i++) {
            ...
            /* Route-order gating: discard maneuver updates until a route update exists or when the index is too old. */
            /*
             * Gate the whole maneuver slot, not a hand-picked key subset.
             * This also covers z_level, exit/lane fields and future per-slot keys.
             */
            ...
            if (d.has(p + "turn_angle")) {
                ...
                if (v != state.mTurnAngle[i]) {
                    ...
                    /*
                     * The native bridge publishes the iAP2 "exit angle" field under the
                     * legacy key name "turn_angle". Keep an explicit copy so side-streets
                     * mapping can use reference naming.
                     */
            ...
            if (d.has(p + "distance")) {
                ...
                if (v != state.mDistance[i]) {
                    state.mDistance[i] = v;
                    /*
                     * The reference renderer computes bargraph in the maneuver-update path (updateCurrentManeuver)
    ...
    private boolean canProcessManeuverIndex(int idx) {
        ...
        /* Storage and presentation are intentionally separate.  A real 0x5202 may preload a future
         * maneuver before 0x5201 supplies an authoritative maneuver_list.  Cache the slot
         * fields now; BAPBridge still presents only slots selected by maneuverOrder. */
        return true;
    }
