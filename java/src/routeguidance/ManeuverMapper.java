 ...
 *
 * Maps iAP2 maneuver semantics to the Audi BAP constants used by the K2161
 * LSD navigation service.
 *
 */
...
public class ManeuverMapper {
    ...
    /*
     * Reference renderer data uses mainElement=44 for ferry, while the BAP constants
     * available to this K2161 build stop at 38. Ferry therefore falls back to
     * FOLLOW_STREET.
     */
    ...
    /*
     * Reference renderer values (as logged by sub_2B5B30):
     * - junctionType: 0=single intersection, 1=roundabout
     * - drivingSide:  0=right-hand traffic, 1=left-hand traffic
     *
     * The native bridge passes the raw iAP2 values through; observed packets match these values.
     */
     ...
     *
     * Verified against reference route-guidance type-info strings
     * (CDIONavigationMetadataTypeInfo::toString(EManeuverType)):
     *   0=NO_TURN, 1=LEFT_TURN, 2=RIGHT_TURN, ... , 53=CHANGE_HIGHWAY_RIGHT
     *
     * RouteGuidance.State initializes "missing" types to -1, so MT_NOT_SET remains outside
    ...
    public static final int MT_FOLLOW_ROAD                = 5;   /* "CONTINUE" in the reference implementation */
    ...
    public static final int MT_START_ROUTE                = 11;  /* "PROCEED_TO_BEGINNING_OF_ROUTE" in the reference implementation */
    public static final int MT_ARRIVE_AT_DESTINATION      = 12;  /* "ARRIVE" in the reference implementation */
    ...
    public static final int MT_START_ROUTE_WITH_U_TURN    = 18;  /* "UTURN_PROCEED_TO_ROUTE" in the reference implementation */
    public static final int MT_U_TURN_AT_ROUNDABOUT       = 19;  /* "ROUNDABOUT_UTURN" in the reference implementation */
    public static final int MT_LEFT_TURN_AT_END           = 20;  /* "TURN_LEFT_END_OF_ROAD" in the reference implementation */
    public static final int MT_RIGHT_TURN_AT_END          = 21;  /* "TURN_RIGHT_END_OF_ROAD" in the reference implementation */
    public static final int MT_HIGHWAY_OFF_RAMP_LEFT      = 22;
    public static final int MT_HIGHWAY_OFF_RAMP_RIGHT     = 23;
    public static final int MT_ARRIVE_DESTINATION_LEFT    = 24;  /* "ARRIVE_LEFT" in the reference implementation */
    public static final int MT_ARRIVE_DESTINATION_RIGHT   = 25;  /* "ARRIVE_RIGHT" in the reference implementation */
     ...
     * @param turnAngle    JunctionElementExitAngle (signed). In the reference implementation this is used for end-of-road / ramps.
    ...
    public static int[] map(int maneuverType, int turnAngle, int junctionType, int drivingSide) {
         ...
         * (follow-road, arrived, ferry, etc). The same ordering is preserved below.
         */

        /* Explicit "not set" sentinel from RouteGuidance.State initialization. */
        ...
        if (maneuverType == MT_ENTER_ROUNDABOUT) {
            /* The reference implementation maps ENTER_ROUNDABOUT to TURN left/right depending on driving side. */
            mainElement = TURN;
        ...
        if (junctionType != JUNCTION_SINGLE_INTERSECTION) {
            ...
            /* For any other maneuverType while junctionType==1, the reference implementation returns NO_INFO. */
            mainElement = NO_INFO;
            direction = DIR_STRAIGHT;
            return new int[] { mainElement, direction };
        }
        ...
        switch (maneuverType) {
            ...
            case MT_KEEP_LEFT:
                /* The reference implementation maps KEEP_LEFT and SLIGHT_LEFT to TURN+SLIGHT_LEFT */
                mainElement = TURN;
            ...
            case MT_KEEP_RIGHT:
                /* The reference implementation maps KEEP_RIGHT and SLIGHT_RIGHT to TURN+SLIGHT_RIGHT */
                mainElement = TURN;
            ...
            case MT_OFF_RAMP:
                 ...
                 * missing-angle fallback for the side. SLIGHT is not hardcoded: a
            ...
            case MT_ARRIVE_DESTINATION_RIGHT:
                /* The reference implementation uses ARRIVED + left/right direction */
                mainElement = ARRIVED;
            ...
            default:
                /* Reference implementation sub_2B9C90: unknown junctionType gate -> NO_INFO(1) */
                mainElement = NO_INFO;
                direction = DIR_STRAIGHT;
                break;
        }
        ...
        /*
         * The reference BAP implementation applies an additional direction override for most actions
         ...
         *
         * CarPlay iAP2 does not expose "Action"/"Turn" enums here, so the
     ...
     * real angle, the same slight/normal/sharp bucketing used for end-of-road
    ...
    private static int applyDsiNavBapDirectionOverride(int maneuverType, int dir) {
         ...
         * SHARP_LEFT/RIGHT must also be preserved — the reference implementation keeps them as-is. */
    ...
    /**
     * Reference roundabout exit direction encoding uses 16-step values (0..240 step 16),
     * derived by quantizing the exit angle to the nearest multiple of 22.5 degrees.
     *
     * Table extracted from reference renderer initialization (unk_530F68 + unk_531010).
     */
    public static int directionFromAngle16(int angle) {
        ...
        /*
         * The reference renderer quantizes by picking the closest entry from the fixed
     ...
     * Reference implementation sub_2B97D0 maps angle -> 8-direction, then the override
     * coarsens to straight/left/right. The coarse mapping is applied directly.
     */
    ...
    private static int dirFromEndOfRoadAngleLeft(int exitAngle) {
        /*
         * Reference implementation logic (sub_2B9C90, MANEUVER_TYPE_TURN_LEFT_END_OF_ROAD):
         * - default: LEFT
    ...
    private static int dirFromEndOfRoadAngleRight(int exitAngle) {
        /*
         * Reference implementation logic (sub_2B9C90, MANEUVER_TYPE_TURN_RIGHT_END_OF_ROAD):
         * - default: RIGHT
