/*
 * Side-street computation for turn-by-turn rendering.
 *
...
final class SideStreets {
    ...
    /* Sentinel used by the reference implementation when the exit-angle field is missing */
    private static final int EXIT_ANGLE_MISSING = 1000;

    /*
     * Some maneuver types use fixed single-entry side-street patterns in the reference implementation.
    ...
    static int[] calcSideStreetDirections(int maneuverType, int junctionType, int drivingSide, int[] junctionAngles, int exitAngle) {
        /*
         * Fixed side-street patterns from the reference implementation.
         */
        if (maneuverType == 7) {
        ...
        /*
         * Dynamic side streets are computed only for specific maneuver types.
    ...
    /*
     * The reference renderer uses a std::map<float,int> populated from a table:
    ...
    /*
     * Reference whitelist: only these maneuver types trigger dynamic
